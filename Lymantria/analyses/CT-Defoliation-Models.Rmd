---
title: "CT Defoliation Model Comparison"
...
mean.scores <- read.csv("data/ct-mean-scores.csv")
...
mean.scores <- mean.scores %>% mutate(year = as.character(year)) %>% rename(Year = year)
...
# Combine datasets using a join on both the PointID and Year columns
ly.comb <- mean.scores %>% 
...
sr.pred <- expand.grid(
    ...
    #range(ly.comb$m_sr_2000.2010_h13_16d)
    m_sr_2000.2010_h13_16d = seq(-2.6, 2.9, by = 0.25),
...
tcg.pred <- expand.grid(
    ...
    #range(ly.comb$m_tcg_2000.2010_h13_full)
    m_tcg_2000.2010_h13_full = seq(-3, 2.25, by = 0.25),
...
model.scores <- ly.comb %>% 
    ...
    # This model would need to be run separately, only for 2017
    select(-m_paper) %>%
...
We actually get two R<sup>2</sup> numbers for these models, a marginal and a conditional R<sup>2</sup>: 
...
```{r Burlap_sr_Param, echo = F, comment = ""}

# Plot fixed effects
burlap.sr.param <- plot_model(burlapmodels[['m_sr_2000.2010_h13_16d']], 
...
```{r Burlap_sr_Pred, echo = F, comment = ""}
...
burlap.sr <- sr.pred %>%
    ...
    augment.conf(burlapmodels[['m_sr_2000.2010_h13_16d']])


# Make a prediction plot for sr
burlap.sr.pred <- burlapmodels[['m_sr_2000.2010_h13_16d']] %>%
  
    # Augment the actual data, adding fitted values and residuals
    augment(type.predict = "response") %>%
    
    # Plot defoliation against model fitted values of abundance
    ggplot(aes(x = m_sr_2000.2010_h13_16d, y = .fitted, color = Year)) +
...
```{r Burlap_sr_Combined, echo = F, comment = "", fig.height = 5, fig.width = 10}

# Combine plots
ggdraw() + 
    
    # Add the two plots
    draw_plot(burlap.sr.param + ggtitle(""), 0, 0, 0.4, 0.95) +
    draw_plot(burlap.sr.pred + ggtitle(""), 0.4, 0, 0.6, 0.95) + 
...
```{r Burlap_tcg_Param, echo = F, comment = ""}

# Plot fixed effects
burlap.tcg.param <- plot_model(burlapmodels[['m_tcg_2000.2010_h13_full']], 
    type = "est", show.values = T, show.p = T, vline.color = "gray60", 
    transform = NULL, value.offset = 0.2, , 
    axis.labels = c("Score:Year 2018", "Year 2018", "Score")) + 
...
```{r Burlap_tcg_Pred, echo = F, comment = ""}
...
burlap.tcg <- tcg.pred %>%
    ...
    # Augment with fitted values and confidence intervals
    augment.conf(burlapmodels[['m_tcg_2000.2010_h13_full']])


# Make a prediction plot for sr
burlap.tcg.pred <- burlapmodels[['m_tcg_2000.2010_h13_full']] %>%
  
    # Augment the actual data, adding fitted values and residuals
    augment(type.predict = "response") %>%
    
    # Plot defoliation against model fitted values of abundance
    ggplot(aes(x = m_tcg_2000.2010_h13_full, y = .fitted, color = Year)) +
      
        # Add model prediction trend line and 95% confidence intervals
        geom_smooth(aes(y = fit, ymin = lower, ymax = upper), size = 1.5, alpha = 0.15, 
            stat = "identity", data = burlap.tcg) +
...
```{r Burlap_tcg_Combined, echo = F, comment = "", fig.height = 5, fig.width = 10}

# Combine plots
ggdraw() + 
    
    # Add the two plots
    draw_plot(burlap.tcg.param + ggtitle(""), 0, 0, 0.4, 0.95) +
    draw_plot(burlap.tcg.pred + ggtitle(""), 0.4, 0, 0.6, 0.95) + 
...
# Plot fixed effects
egg.sr.param <- plot_model(eggmodels[['m_sr_2000.2010_h13_16d']], 
    type = "est", show.values = T, show.p = T, vline.color = "gray60", 
    transform = NULL, value.offset = 0.2, , 
    axis.labels = c("Score:Year 2018", "Year 2018", "Score")) + 
...
egg.sr <- sr.pred %>%
    ...
    # Augment with fitted values and confidence intervals
    augment.conf(eggmodels[['m_sr_2000.2010_h13_16d']])


# Make a prediction plot for sr
egg.sr.pred <- eggmodels[['m_sr_2000.2010_h13_16d']] %>%
...
# Plot fixed effects
egg.tcg.param <- plot_model(eggmodels[['m_tcg_2000.2010_h13_full']], 
    type = "est", show.values = T, show.p = T, vline.color = "gray60", 
    transform = NULL, value.offset = 0.2, , 
    axis.labels = c("Score:Year 2018", "Year 2018", "Score")) + 
...
egg.tcg <- tcg.pred %>%
    ...
    # Augment with fitted values and confidence intervals
    augment.conf(eggmodels[['m_tcg_2000.2010_h13_full']])


# Make a prediction plot for sr
egg.tcg.pred <- eggmodels[['m_tcg_2000.2010_h13_full']] %>%
...
AICtab(m_egg.pred, burlapmodels[['m_tcg_2000.2010_h13_full']], 
    burlapmodels[['m_sr_2000.2010_h13_16d']], 
    base = TRUE, weights = TRUE, mnames = c("m_egg.pred", 
    "m_tcg_2000.2010_h13_full", "m_sr_2000.2010_h13_16d")) %>% 
...
egg.predictor.param <- plot_model(m_egg.pred, 
    ...
    # Add title, axis label
    labs(y = "Parameter Estimate", title = "Egg Mass Predictor") +
...
ggdraw() + 
    ...
    # Add title
    draw_plot_label("Egg Mass Predictor", 0.1, 1, size = 20)
...
ly.comb %>% 
    filter(Year == 2017) %>%
    gather(model, score, m_reanalysis, m_tcg_2000.2010_h13_full) %>%
    ggplot(aes(x = score)) + 
...
ly.comb %>%
    ...
    pivot_longer(cols = c(m_sr_2000.2010_h13_16d, m_tcg_2000.2010_h13_full), 
