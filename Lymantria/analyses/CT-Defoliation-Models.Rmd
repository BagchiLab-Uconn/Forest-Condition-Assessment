...
This analysis compares Val Pasquarella's defoliation data from Landsat satellite to 

Here, we examine how varying the following components of the Landsat change-in-condition model affect how well that predicts defoliation on the ground:
...
# Read in mean condition scores for 2017
scores.2017 <- read.csv("data/plots_ct_burlap_2017_v2-2.csv") %>%
    setNames(str_remove(names(.), '_201\\d_05_01_201\\d_09_30'))

# Read in mean condtion scores for 2018
scores.2018 <- read.csv("data/plots_ct_burlap_2018_v2-2.csv") %>%
     setNames(str_remove(names(.), '_201\\d_05_01_201\\d_09_30'))
...
sr.pred <- expand.grid(
    ...
    m_sr_2000_2010_h13_16d = seq(-2.6, 2.9, by = 0.25),
...
tcg.pred <- expand.grid(
    ...
    m_tcg_2000_2010_h13_full = seq(-3, 2.25, by = 0.25),
...
# Plot fixed effects
burlap.sr.param <- plot_model(burlapmodels[['m_sr_2000_2010_h13_16d']], 
...
burlap.sr <- sr.pred %>%
    ...
    augment.conf(burlapmodels[['m_sr_2000_2010_h13_16d']])


# Make a prediction plot for sr
burlap.sr.pred <- burlapmodels[['m_sr_2000_2010_h13_16d']] %>%
...
# Plot fixed effects
burlap.tcg.param <- plot_model(burlapmodels[['m_tcg_2000_2010_h13_full']], 
    type = "est", show.values = T, show.p = T, vline.color = "gray60", 
    transform = NULL, value.offset = 0.2, , 
    axis.labels = c("Score:Year 2018", "Year 2018", "Score")) + 
...
burlap.tcg <- tcg.pred %>%
    ...
    # Augment with fitted values and confidence intervals
    augment.conf(burlapmodels[['m_tcg_2000_2010_h13_full']])


# Make a prediction plot for sr
burlap.tcg.pred <- burlapmodels[['m_tcg_2000_2010_h13_full']] %>%
...
AICtab(m_egg.pred, burlapmodels[['m_tcg_2000_2010_h13_full']], 
    burlapmodels[['m_sr_2000_2010_h13_16d']], 
