...
AIC.quabbin <- r2.quabbin %>% 
    ...
    # Add in the AIC data
    left_join(AIC.quabbin %>% rename(baseline = model), by = "baseline") %>%
...
quabbin.top.param <- plot_model(QuabModBeta[['m_tcg_2005_2015_h13_full']], 
    ...
    transform = NULL, value.offset = 0.2, value.size = 6, 
    axis.labels = c("Score")) + 
...
AIC.burlap <- r2.burlap %>% 
    ...
    # Add in the AIC data
    left_join(AIC.burlap %>% rename(baseline = model), by = "baseline") %>%
...
AIC.burlap.17 <- r2.burlap.17 %>% 
    ...
    # Add in the AIC data
    left_join(AIC.burlap.17 %>% rename(baseline = model), by = "baseline") %>%
...
# Write to CSV
write.csv(AIC.burlap.17, file = "data/CTBurlap32ModelsAIC_2017.csv", row.names = FALSE)
...
AIC.burlap.18 <- r2.burlap.18 %>% 
    ...
    # Add in the AIC data
    left_join(AIC.burlap.18 %>% rename(baseline = model), by = "baseline") %>%
...
# Write to CSV
write.csv(AIC.burlap.18, file = "data/CTBurlap32ModelsAIC_2018.csv", row.names = FALSE)
...
burlap.consensus.pred <- burlapmodels[['m_reanalysis']] %>%
    ...
    ggplot(aes(x = m_reanalysis, y = .fitted, color = Year)) +
        ...
        # Add model prediction trend line and 95% confidence intervals
        geom_smooth(aes(y = fit, ymin = lower, ymax = upper), size = 1.5, 
            alpha = 0.15, stat = "identity", data = burlap.consensus) +
...
#### Top Burlap Baseline: sr_2000_2010_h13_16d


```{r Burlap_Top_Param_2017, echo = F, comment = ""}

# Plot fixed effects
burlap.top.param.17 <- plot_model(burlapmodels17[['m_sr_2000_2010_h13_16d']], 
    type = "est", show.values = T, show.p = T, vline.color = "gray60", 
    transform = NULL, value.offset = 0.2, value.size = 6, 
    axis.labels = c("Score")) + 
    
    # Add title, axis label
    labs(y = "Parameter Estimate", title = "Baseline: sr_2000_2010_h13_16d") +
...
```{r Burlap_Top_Pred_2017, echo = F, comment = ""}

# Make a dummy dataset for interaction/prediction plots for Burlap larva top model
burlap.top.17 <- expand.grid(
    
    # Range of scoresfrom the CT data, adjust as needed
    #range(ly.comb$m_sr_2000_2010_h13_16d[ly.comb$Year == 2017])
    m_sr_2000_2010_h13_16d = seq(-2.95, 1.75, by = 0.1),
    
    # Model without random factors (the mean random effect)
    BlockID = NA, SiteID = NA) %>%
  
    # Augment with fitted values and confidence intervals
    augment.conf(burlapmodels17[['m_sr_2000_2010_h13_16d']])


# Make a prediction plot for burlap top model
burlap.top.pred.17 <- burlapmodels17[['m_sr_2000_2010_h13_16d']] %>%
  
    # Augment the actual data, adding fitted values and residuals
    augment(type.predict = "response") %>%
    
    # Plot defoliation against model fitted values of abundance
    ggplot(aes(x = m_sr_2000_2010_h13_16d, y = .fitted)) +
        
        # Add fitted data values as scatterplot
        geom_point(alpha = 0.9) +
        
        # Add errorbars to the fitted values equal to the residuals
        geom_errorbar(aes(ymin = .fitted - .resid, ymax = .fitted + .resid), alpha = 0.5) +
  
        # Add model prediction trend line and 95% confidence intervals
        geom_smooth(aes(y = fit, ymin = lower, ymax = upper), size = 1.5, 
            alpha = 0.15, stat = "identity", data = burlap.top.17) +
        
        # Add axis labels
        labs(x = "Defoliation Score", y = "Fitted Larval Abundance", 
            title = "Baseline: sr_2000_2010_h13_16d") +
...
```{r Burlap_Top_Combined_2017, echo = F, comment = "", fig.height = 5, fig.width = 10}

# Combine plots
ggdraw() + 
    
    # Add the two plots
    draw_plot(burlap.top.param.17 + ggtitle(""), 0, 0, 0.4, 0.95) +
    draw_plot(burlap.top.pred.17 + ggtitle(""), 0.4, 0, 0.6, 0.95) + 
    
    # Add subplot labels
    #draw_plot_label(c("A", "B"), c(0.11, 0.45), c(0.9, 0.9), size = 24) +
  
    # Add title
    draw_plot_label("Baseline: sr_2000_2010_h13_16d", 0.1, 1, size = 20)
...
AIC.egg <- r2.egg %>% 
    ...
    # Add in the AIC data
    left_join(AIC.egg %>% rename(baseline = model), by = "baseline") %>%
...
AIC.egg.17 <- r2.egg.17 %>% 
    ...
    # Add in the AIC data
    left_join(AIC.egg.17 %>% rename(baseline = model), by = "baseline") %>%
...
# Write to CSV
write.csv(AIC.egg.17, file = "data/CTEggMasses32ModelsAIC_2017.csv", 
    row.names = FALSE)
...
AIC.egg.18 <- r2.egg.18 %>% 
    ...
    # Add in the AIC data
    left_join(AIC.egg.18 %>% rename(baseline = model), by = "baseline") %>%
...
# Write to CSV
write.csv(AIC.egg.18, file = "data/CTEggMasses32ModelsAIC_2018.csv", 
    row.names = FALSE)
...
#### Top Egg Mass Baseline: sr_2000-2010_h13_16d


```{r Egg_Top_Param, echo = F, comment = ""}

# Plot fixed effects
egg.top.param <- plot_model(eggmodels[['m_sr_2000_2010_h13_16d']], 
    type = "est", show.values = T, show.p = T, vline.color = "gray60", 
    transform = NULL, value.offset = 0.2, value.size = 6, 
    axis.labels = c("Score:Year 2018", "Year 2018", "Score")) + 
...
```{r Egg_Top_Pred, echo = F, comment = ""}


# Make a dummy dataset for interaction/prediction plots for egg mass top model
egg.top <- expand.grid(
    
    # Range of scoresfrom the CT data, adjust as needed
    #range(ly.comb$m_sr_2000_2010_h13_16d)
    m_sr_2000_2010_h13_16d = seq(-2.95, 2.3, by = 0.25),
...
# Make a prediction plot for egg mass top model
egg.top.pred <- eggmodels[['m_sr_2000_2010_h13_16d']] %>%
    ...
    ggplot(aes(x = m_sr_2000_2010_h13_16d, y = .fitted, color = Year)) +
        ...
        # Add model prediction trend line and 95% confidence intervals
        geom_smooth(aes(y = fit, ymin = lower, ymax = upper), size = 1.5, 
            alpha = 0.15, stat = "identity", data = egg.top) +
...
```{r Egg_Top_Combined, echo = F, comment = "", fig.height = 5, fig.width = 10}

# Combine plots
ggdraw() + 
    
    # Add the two plots
    draw_plot(egg.top.param + ggtitle(""), 0, 0, 0.4, 0.95) +
    draw_plot(egg.top.pred + ggtitle(""), 0.4, 0, 0.6, 0.95) + 
...
### Top Egg Mass Baseline: tcg_2000_2010_h12_full


```{r Egg_Top_Param_2017, echo = F, comment = ""}

# Plot fixed effects
egg.top.param.17 <- plot_model(eggmodels17[['m_tcg_2000_2010_h12_full']], 
    type = "est", show.values = T, show.p = T, vline.color = "gray60", 
    transform = NULL, value.offset = 0.2, value.size = 6, 
    axis.labels = c("Score")) + 
    
    # Add title, axis label
    labs(y = "Parameter Estimate", title = "Baseline: tcg_2000_2010_h12_full") +
...
```{r Egg_Top_Pred_2017, echo = F, comment = ""}


# Make a dummy dataset for interaction/prediction plots for egg mass top model
egg.top.17 <- expand.grid(
    
    # Range of scoresfrom the CT data, adjust as needed
    #range(ly.comb$m_tcg_2000_2010_h12_full[ly.comb$Year == 2017])
    m_tcg_2000_2010_h12_full = seq(-3.35, 0.4, by = 0.25),
    
    # Model without random factors (the mean random effect)
    BlockID = NA, SiteID = NA) %>%
  
    # Augment with fitted values and confidence intervals
    augment.conf(eggmodels17[['m_tcg_2000_2010_h12_full']])


# Make a prediction plot for egg mass top model
egg.top.pred.17 <- eggmodels17[['m_tcg_2000_2010_h12_full']] %>%
  
    # Augment the actual data, adding fitted values and residuals
    augment(type.predict = "response") %>%
    
    # Plot defoliation against model fitted values of abundance
    ggplot(aes(x = m_tcg_2000_2010_h12_full, y = .fitted)) +
        ...
        # Add model prediction trend line and 95% confidence intervals
        geom_smooth(aes(y = fit, ymin = lower, ymax = upper), size = 1.5, 
            alpha = 0.15, stat = "identity", data = egg.top.17) +
        
        # Add axis labels
        labs(x = "Defoliation Score", y = "Fitted Egg Masses", 
            title = "Baseline: tcg_2000_2010_h12_full") +
...
```{r Egg_Top_Combined_2017, echo = F, comment = "", fig.height = 5, fig.width = 10}

# Combine plots
ggdraw() + 
    
    # Add the two plots
    draw_plot(egg.top.param.17 + ggtitle(""), 0, 0, 0.4, 0.95) +
    draw_plot(egg.top.pred.17 + ggtitle(""), 0.4, 0, 0.6, 0.95) + 
    
    # Add subplot labels
    #draw_plot_label(c("A", "B"), c(0.11, 0.45), c(0.9, 0.9), size = 24) +
  
    # Add title
    draw_plot_label("Baseline: tcg_2000_2010_h12_full", 0.1, 1, size = 20)
...
m_egg.pred <- glmmTMB(Abundance ~ EggMasses * Year + (1|SiteID), 
...
```{r Egg_Predictor_Model_2017, echo = F, comment = ""}

# Predict larval abundance with egg mass counts
m_egg.pred.17 <- glmmTMB(Abundance ~ EggMasses + (1|SiteID), 
    family = nbinom2, data = ly.comb %>% filter(Year == 2017))

summary(m_egg.pred.17)
```

```{r Egg_Predictor_Model_2018, echo = F, comment = ""}

# Predict larval abundance with egg mass counts
m_egg.pred.18 <- glmmTMB(Abundance ~ EggMasses + (1|SiteID), 
    family = nbinom2, data = ly.comb %>% filter(Year == 2018))

summary(m_egg.pred.18)
...
```{r Egg_Predictor_AIC_2017, echo = F, comment = ""}


AICtab(m_egg.pred.17, 
    burlapmodels17[['m_sr_2000_2010_h13_16d']], 
    burlapmodels17[['m_tcg_2005_2015_h13_full']], 
    burlapmodels17[['m_tcg_2000_2010_h12_full']],
    base = TRUE, weights = TRUE, mnames = c("Predict abundance with egg masses", 
    "sr_2000_2010_h13_16d", "tcg_2005_2015_h13_full", 
    "tcg_2000_2010_h12_full")) %>% 
...
```{r Egg_Predictor_AIC_2018, echo = F, comment = ""}


AICtab(m_egg.pred.18, 
    burlapmodels18[['m_ndvi_2000_2010_h13_16d']], 
    burlapmodels18[['m_tcg_2005_2015_h13_full']], 
    burlapmodels18[['m_tcg_2005_2015_h13_16d']],
    base = TRUE, weights = TRUE, mnames = c("Predict abundance with egg masses", 
    "ndvi_2000_2010_h13_16d", "tcg_2005_2015_h13_full", 
    "tcg_2005_2015_h13_16d")) %>% 
...
```{r Egg_Predictor_R2_2017, echo = F, comment = ""}
...
```{r Egg_Predictor_R2_2018, echo = F, comment = ""}
...
egg.predictor.pred <- m_egg.pred %>%
    ...
    ggplot(aes(x = EggMasses, y = .fitted, color = Year)) +
        ...
        # Add model prediction trend line and 95% confidence intervals
        geom_smooth(aes(y = fit, ymin = lower, ymax = upper), size = 1.5, 
            alpha = 0.15, stat = "identity", data = egg.predictor) +
...
```{r Egg_Predictor_Param_2017, echo = F, comment = ""}

# Plot fixed effects
egg.predictor.param.17 <- plot_model(m_egg.pred.17, 
    type = "est", show.values = T, show.p = T, plvline.color = "gray60", 
    transform = NULL, value.offset = 0.2,
    axis.labels = c("Egg Masses")) + 
...
# Display Plot
#egg.predictor.param.17


```

```{r Egg_Predictor_Pred_2017, echo = F, comment = ""}

# Make a dummy sr dataset for interaction/prediction plots
egg.predictor.17 <- expand.grid(
  
    # Range of egg mass counts
    #range(ly.comb$EggMasses[ly.comb$Year == 2017], na.rm = T)
    EggMasses = seq(0, 181, by = 20),
    
    # Model without random factors (the mean random effect)
    BlockID = NA, SiteID = NA) %>%

    # Augment with fitted values and confidence intervals
    augment.conf(m_egg.pred.17)


# Make a prediction plot for sr
egg.predictor.pred.17 <- m_egg.pred.17 %>%
  
    # Augment the actual data, adding fitted values and residuals
    augment(type.predict = "response") %>%
    
    # Plot defoliation against model fitted values of abundance
    ggplot(aes(x = EggMasses, y = .fitted)) +
        ...
        # Add model prediction trend line and 95% confidence intervals
        geom_smooth(aes(y = fit, ymin = lower, ymax = upper), size = 1.5, 
            alpha = 0.15, stat = "identity", data = egg.predictor.17) +
...
# Show plot
egg.predictor.pred.17

```


```{r Egg_Predictor_Combined_2017, echo = F, comment = "", fig.height = 5, fig.width = 10}

# Combine plots
ggdraw() + 
    
    # Add the two plots
    draw_plot(egg.predictor.param.17 + ggtitle(""), 0, 0, 0.4, 0.95) +
    draw_plot(egg.predictor.pred.17 + ggtitle(""), 0.4, 0, 0.6, 0.95) + 
...
```{r Panel_Plot_Top, echo = F, comment = "", fig.height = 12, fig.width = 8}

# Combine plots
ggdraw() + 
    
    # Add the Quabbin plots
    draw_plot(quabbin.top.param + ggtitle(""), 0, 0.66, 0.4, 0.33) +
    draw_plot(quabbin.top.pred, 0.4, 0.66, 0.6, 0.33) + 
  
    # Add the burlap larva plots
    draw_plot(burlap.top.param.17 + ggtitle(""), 0, 0.33, 0.4, 0.33) +
    draw_plot(burlap.top.pred.17, 0.4, 0.33, 0.6, 0.33) + 
  
    # Add the egg mass plots
    draw_plot(egg.top.param.17 + ggtitle(""), 0, 0, 0.4, 0.33) +
    draw_plot(egg.top.pred.17, 0.4, 0, 0.6, 0.33) +
    
    # Add subplot labels
    draw_plot_label(c("A", "B", "C", "D", "E", "F"), 
        c(0.04, 0.37, 0.04, 0.37, 0.04, 0.37), 
        c(0.99, 0.99, 0.66, 0.66, 0.33, 0.33), size = 24)
...
```{r Combined_Param_Plot_Top, echo = F, comment = ""}

burlap.coef <- tidy(burlapmodels17[['m_sr_2000_2010_h13_16d']]) %>% 
    filter(term == 'm_sr_2000_2010_h13_16d') %>% 
    mutate(data = "CT Larval Abundance\nsr_2000_2010_h13_16d")

egg.coef <- tidy(burlapmodels17[['m_tcg_2000_2010_h12_full']]) %>% 
    filter(term == 'm_tcg_2000_2010_h12_full') %>% 
    mutate(data = "CT Egg Masses\ntcg_2000_2010_h12_full")

quabbin.coef <- tidy(QuabModBeta[['m_tcg_2005_2015_h13_full']]) %>% 
    filter(term == 'm_tcg_2005_2015_h13_full') %>% 
    mutate(data = "Quabbin Defoliation\ntcg_2005_2015_h13_full")


combined.param.top <- burlap.coef %>% rbind(egg.coef, quabbin.coef) %>%
  
    mutate(
        lower = estimate - qnorm(0.025) * std.error,
        upper = estimate + qnorm(0.025) * std.error,
    ...
    ggplot(aes(x = estimate, y = data)) +
        ...
        # Add forest plot
        geom_pointrange(aes(xmin = lower, xmax = upper), color = "red") +
  
        # Add a vertical line at zero
        geom_vline(aes(xintercept = 0), color = "gray60", size = 1) +
  
        # Add the parameter estimate numbers, with significance
        geom_text(aes(label = paste(round(estimate, 2), sig)), 
...
insight::get_variance(eggmodels17[['m_reanalysis']], name_fun = "r2()", name_full = "r-squared")

summary(eggmodels[['m_reanalysis']])
summary(eggmodels[['m_tcg_2000_2010_h12_full']])
```


```{r Panel_Plot_Top_Alt, echo = F, comment = "", fig.height = 12, fig.width = 6}


# Combine plots
ggdraw() + 
    
    # Add the Quabbin plot
    draw_plot(quabbin.top.pred + labs(x = "", y = "Ground Defoliation"), 
        0.0, 0.73, 1, 0.25) + 
  
    # Add the burlap larva plot
    draw_plot(burlap.top.pred.17 + labs(x = "", y = "Larval Abundance"), 
        0.0, 0.49, 1, 0.25) + 
  
    # Add the egg mass plot
    draw_plot(egg.top.pred.17 + labs(y = "Egg Masses"), 0.0, 0.26, 1, 0.25) +
    
    # Add the parameter forest plot
    draw_plot(combined.param.top, 0.0, 0.0, 1, 0.25) +
  
    # Add subplot labels
    draw_plot_label(c("A", "B", "C", "D"), 
        c(0.0, 0.0, 0.0, 0.0), 
        c(1, 0.76, 0.52, 0.27), size = 24)
